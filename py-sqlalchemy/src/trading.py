from sqlalchemy import create_engine
from sqlalchemy.orm import Session
from trades.user import User, get_users
from trades.product import Product, get_products, get_product_by_product_code
from trades.trade import Trade, Side
from datetime import date, timedelta

def seed(db: Session):
    users = [
        User(name="Joe", fullname="Joe Bloggs", nickname=None)
    ]
    db.add_all(users)
    products = [
        Product(product_code="AUD/USD"),
        Product(product_code="XAU/USD"),
        Product(product_code="NZD/USD"),
        Product(product_code="JPY/USD"),
    ]
    db.add_all(products)
    today = date.today()
    yesterday = today - timedelta(1)
    trades = [
        Trade(product=products[0], business_date=today, side=Side.B, qty=10),
        Trade(product=products[0], business_date=today, side=Side.S, qty=30),
        Trade(product=products[0], business_date=yesterday, side=Side.B, qty=40),
        Trade(product=products[1], business_date=today, side=Side.S, qty=20)
    ]
    db.add_all(trades)
    db.commit()

def run(db: Session):
    seed(db) 

    product = get_product_by_product_code(db, "AUD/USD")
    if product:
        [print(trade) for trade in product.trades]

    users = get_users(db)
    print(users)
    [print(product) for product in get_products(db)]
